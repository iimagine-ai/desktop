...
const FolderConnectUI = {
      ...
      <div class="fc-folder bg-white/50 dark:bg-neutral-800/50 border border-neutral-200/40 dark:border-neutral-700/40 rounded-xl p-3 shadow-sm" data-id="${folder.id}">
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0 flex-1">
            ...
            <div id="fc-embed-${folder.id}" class="hidden mt-1.5 pl-4">
              <div class="flex items-center gap-2">
                <div class="flex-1 bg-neutral-100 dark:bg-neutral-700 rounded-full h-1.5">
                  <div class="fc-embed-bar bg-blue-500 h-1.5 rounded-full transition-all" style="width: 0%"></div>
                </div>
                <span class="fc-embed-label text-[10px] text-blue-500 dark:text-blue-400 tabular-nums"></span>
              </div>
            </div>
            <div id="fc-cortex-${folder.id}" class="hidden mt-1.5 pl-4">
              <div class="flex items-center gap-2">
                <div class="flex-1 bg-neutral-100 dark:bg-neutral-700 rounded-full h-1.5">
                  <div class="fc-cortex-bar bg-purple-500 h-1.5 rounded-full transition-all" style="width: 0%"></div>
                </div>
                <span class="fc-cortex-label text-[10px] text-purple-500 dark:text-purple-400 tabular-nums"></span>
              </div>
            </div>
          </div>
  ...
  _bindEvents() {
    ...
    document.querySelectorAll('.fc-process-kg').forEach(btn => {
      btn.addEventListener('click', async () => {
        const id = btn.dataset.id;
        btn.disabled = true;
        btn.textContent = 'Processing...';
        const result = await window.api.folders.processKG(id);
        if (result?.error) {
          alert(result.error);
        }
        btn.disabled = false;
  ...
  _listenForProgress() {
    ...
    window.api.folders.onProgress((data) => {
      ...
      if (data.done) {
        ...
        el.innerHTML = `
          <div class="flex items-center gap-1.5 text-[11px] text-emerald-600 dark:text-emerald-400">
            ...
            <span>Synced — ${data.indexed || data.total || 0} files indexed</span>
          </div>
        `;
        setTimeout(() => {
      ...
      el.innerHTML = `
        <div class="flex items-center gap-2">
          <div class="flex-1 bg-neutral-100 dark:bg-neutral-700 rounded-full h-1.5">
            <div class="bg-neutral-900 dark:bg-neutral-100 h-1.5 rounded-full transition-all" style="width: ${pct}%"></div>
          </div>
          <span class="text-[10px] text-neutral-400 tabular-nums">${data.indexed}/${data.total}</span>
        </div>
      `;
    });

    // Embedding progress
    window.api.folders.onEmbedStart((data) => {
      const el = document.querySelector(`#fc-embed-${data.collectionId}`);
      if (!el) return;
      el.classList.remove('hidden');
      el.innerHTML = `
        <div class="flex items-center gap-2">
          <div class="flex-1 bg-neutral-100 dark:bg-neutral-700 rounded-full h-1.5">
            <div class="bg-blue-500 h-1.5 rounded-full transition-all" style="width: 0%"></div>
          </div>
          <span class="text-[10px] text-blue-500 dark:text-blue-400 tabular-nums">Embedding 0/${data.total}</span>
        </div>
      `;
    });

    window.api.folders.onEmbedProgress((data) => {
      const el = document.querySelector(`#fc-embed-${data.collectionId}`);
      if (!el) return;
      el.classList.remove('hidden');
      ...
      el.innerHTML = `
        <div class="flex items-center gap-2">
          ...
          <span class="text-[10px] text-blue-500 dark:text-blue-400 tabular-nums">Embedding ${data.processed}/${data.total}</span>
        </div>
      `;
    });

    window.api.folders.onEmbedDone((data) => {
      const el = document.querySelector(`#fc-embed-${data.collectionId}`);
      if (!el) return;
      el.innerHTML = `
        <div class="flex items-center gap-1.5 text-[11px] text-blue-600 dark:text-blue-400">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
          <span>${data.embedded} chunks embedded</span>
        </div>
      `;
    ...
    // Cortex KG progress
    window.api.folders.onCortexProgress((data) => {
      const el = document.querySelector(`#fc-cortex-${data.folderId}`);
      if (!el) return;
      el.classList.remove('hidden');

      if (data.phase === 'done') {
        el.innerHTML = `
          <div class="flex items-center gap-1.5 text-[11px] text-purple-600 dark:text-purple-400">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
            <span>${data.processed} files processed into KG</span>
          </div>
        `;
      ...
      if (data.phase === 'start') {
        el.innerHTML = `
          <div class="flex items-center gap-2">
            <div class="flex-1 bg-neutral-100 dark:bg-neutral-700 rounded-full h-1.5">
              <div class="bg-purple-500 h-1.5 rounded-full transition-all" style="width: 0%"></div>
            </div>
            <span class="text-[10px] text-purple-500 dark:text-purple-400 tabular-nums">KG 0/${data.total}</span>
          </div>
        `;
        return;
      }
      ...
      el.innerHTML = `
        <div class="flex items-center gap-2">
          <div class="flex-1 bg-neutral-100 dark:bg-neutral-700 rounded-full h-1.5">
            <div class="bg-purple-500 h-1.5 rounded-full transition-all" style="width: ${pct}%"></div>
          </div>
          <span class="text-[10px] text-purple-500 dark:text-purple-400 tabular-nums">KG ${data.processed}/${data.total}</span>
        </div>
      `;
    });
  },
