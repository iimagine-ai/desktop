...
The desktop companion uses a remote JSON manifest to recommend AI models to users. This replaces the old hardcoded model database with a dynamic, updatable system.
...
2. Include: ollama_id, family, categories, size, RAM requirement, quality tier
3. Run the script to verify it passes the Ollama availability check
4. Commit and push
...
6. User clicks Download → triggers `ollama pull {model_id}` via existing IPC
