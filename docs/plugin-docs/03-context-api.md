...
## context.getEnginePort() — Local Engine Port

Returns the port number for the local inference engine. The engine exposes an OpenAI-compatible API at `http://localhost:${port}/v1/`.

```javascript
const port = context.getEnginePort();
// → e.g. 8847
...
| `context.getEnginePort()` | Function → number | Get local engine port for AI calls |
