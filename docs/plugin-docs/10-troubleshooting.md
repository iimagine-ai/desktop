...
## 400 Errors from the Engine

### Symptoms
- `LLM request failed: 400`
- Plugin works intermittently
...
await fetch(`http://localhost:${enginePort}/v1/chat/completions`, {
  body: JSON.stringify({ messages: [...] })  // missing model field
});

// RIGHT — include model and messages
await fetch(`http://localhost:${enginePort}/v1/chat/completions`, {
  method: 'POST',
...
**No model loaded:**
The engine must have a model loaded before it can serve requests. Check the engine health first:
```javascript
const health = await fetch(`http://localhost:${enginePort}/health`);
if (!health.ok) {
  console.warn('Engine not ready — model may not be loaded');
}
```

### Best Practice

Always check engine health before making calls:
```javascript
...
activate(context) {
  ...
  const enginePort = context.getEnginePort ? context.getEnginePort() : null;
...
| 400 from engine | Check engine health, verify request body format |
