...
module.exports = {
  ...
  activate(context) {
    // context contains: { db, store, kbStorage, getEnginePort }
...
```js
{
  ...
  getEnginePort,    // Function returning the local engine port (for LLM calls via OpenAI-compatible API)
}
```
...
- Use the `getEnginePort()` to make local LLM calls via the OpenAI-compatible API
