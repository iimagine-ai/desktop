...
class Redactor {
  constructor({ getEnginePort, store }) {
    this._getEnginePort = getEnginePort;
    this._store = store;
  ...
  /**
   * Use local iimagine-engine to detect PII entities.
   */
  async _detectWithLLM(text) {
    // Use iimagine-engine completions endpoint
    const response = await fetch('http://127.0.0.1:8847/v1/completions', {
      method: 'POST',
